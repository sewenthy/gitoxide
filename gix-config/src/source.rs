...
#[doc = " The category of a [`Source`], in order of ascending precedence."]
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash, Ord, PartialOrd)]
pub enum Kind {
    #[doc = " A special configuration file that ships with the git installation, and is thus tied to the used git binary."]
    GitInstallation,
    #[doc = " A source shared for the entire system."]
    System,
    #[doc = " Application specific configuration unique for each user of the `System`."]
    Global,
    #[doc = " Configuration relevant only to the repository, possibly including the worktree."]
    Repository,
    #[doc = " Configuration specified after all other configuration was loaded for the purpose of overrides."]
    Override,
}
impl Kind {
    #[doc = " Return a list of sources associated with this `Kind` of source, in order of ascending precedence."]
...
impl Source {
    #[doc = " Return true if the source indicates a location within a file of a repository."]
    ...
    #[doc = " Returns the location at which a file of this type would be stored, or `None` if"]
    #[doc = " there is no notion of persistent storage for this source, with `env_var` to obtain environment variables."]
    #[doc = " Note that the location can be relative for repository-local sources like `Local` and `Worktree`,"]
    #[doc = " and the caller has to known which base it it relative to, namely the `common_dir` in the `Local` case"]
    #[doc = " and the `git_dir` in the `Worktree` case."]
    #[doc = " Be aware that depending on environment overrides, multiple scopes might return the same path, which should"]
    #[doc = " only be loaded once nonetheless."]
    #[doc = ""]
    #[doc = " With `env_var` it becomes possible to prevent accessing environment variables entirely to comply with `gix-sec`"]
    #[doc = " permissions for example."]
    pub fn storage_location(
        ...
        match self {
            ...
            System => env_var("GIT_CONFIG_NO_SYSTEM").is_none().then(|| {
                PathBuf::from(
                    env_var("GIT_CONFIG_SYSTEM").unwrap_or_else(|| "/etc/gitconfig".into()),
                )
                .into()
            }),
...
#[doc = " Environment information involving the `git` program itself."]
mod git {
    use bstr::{BStr, BString, ByteSlice};
    use std::process::{Command, Stdio};
    #[doc = " Returns the file that contains git configuration coming with the installation of the `git` file in the current `PATH`, or `None`"]
    #[doc = " if no `git` executable was found or there were other errors during execution."]
    pub fn install_config_path() -> Option<&'static BStr> {
        static PATH: once_cell::sync::Lazy<Option<BString>> = once_cell::sync::Lazy::new(|| bar());
        PATH.as_ref().map(|b| b.as_ref())
    }
    fn bar() -> Option<BString> {
        let mut cmd = Command::new(if cfg!(windows) { "git.exe" } else { "git" });
        cmd.args(["config", "-l", "--show-origin"])
            .stdin(Stdio::null())
            .stderr(Stdio::null());
        first_file_from_config_with_origin(cmd.output().ok()?.stdout.as_slice().into())
