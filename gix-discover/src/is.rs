...
#[doc = " Returns true if the given `git_dir` seems to be a bare repository."]
#[doc = ""]
#[doc = " Please note that repositories without an index generally _look_ bare, even though they might also be uninitialized."]
...
#[doc = " Returns true if `git_dir` is is located within a `.git/modules` directory, indicating it's a submodule clone."]
...
#[doc = " What constitutes a valid git repository, returning the guessed repository kind"]
#[doc = " purely based on the presence of files. Note that the gix-config ultimately decides what's bare."]
#[doc = ""]
#[doc = " Returns the `Kind` of git directory that was passed, possibly alongside the supporting private worktree git dir."]
#[doc = ""]
#[doc = " Note that `.git` files are followed to a valid git directory, which then requires…"]
#[doc = ""]
#[doc = "   * …a valid head"]
#[doc = "   * …an objects directory"]
#[doc = "   * …a refs directory"]
#[doc = ""]
pub fn git(git_dir: impl AsRef<Path>) -> Result<crate::repository::Kind, crate::is_git::Error> {
            ...
            .and_then(|cd| {
                crate::path::from_plain_file(git_dir.join("gitdir"))
                    ...
                    .map(|worktree_gitfile| {
                        (crate::path::without_dot_git_dir(worktree_gitfile), cd)
                    })
            });
    ...
    }
    {
        ...
        if !objects_path.is_dir() {
            return Err(crate::is_git::Error::MissingObjectsDirectory {
                missing: objects_path,
            });
        }
    }
    {
    ...
    Ok(match kind {
        ...
        Kind::MaybeRepo => {
            if bare(git_dir) {
                ...
                let mut is_submodule_git_dir = bar(git_dir);
