...
#[doc = " The kind of repository to create."]
#[derive(Debug, Copy, Clone)]
pub enum Kind {
    #[doc = " An empty repository with a `.git` folder, setup to contain files in its worktree."]
    WithWorktree,
    #[doc = " A bare repository without a worktree."]
    Bare,
}
...
#[doc = " Options for use in [`into()`];"]
#[derive(Copy, Clone, Default)]
pub struct Options {
    #[doc = " If true, and the kind of repository to create has a worktree, then the destination directory must be empty."]
    #[doc = ""]
    #[doc = " By default repos with worktree can be initialized into a non-empty repository as long as there is no `.git` directory."]
    pub destination_must_be_empty: bool,
    #[doc = " If set, use these filesystem capabilities to populate the respective gix-config fields."]
    #[doc = " If `None`, the directory will be probed."]
    pub fs_capabilities: Option<gix_worktree::fs::Capabilities>,
}
#[doc = " Create a new `.git` repository of `kind` within the possibly non-existing `directory`"]
#[doc = " and return its path."]
#[doc = " Note that this is a simple template-based initialization routine which should be accompanied with additional corrections"]
#[doc = " to respect git configuration, which is accomplished by [its callers][crate::ThreadSafeRepository::init_opts()]"]
#[doc = " that return a [Repository][crate::Repository]."]
pub fn into(
    ...
    create_dir(&dot_git)?;
    {
        let mut cursor = bar____EXTRACT_THIS(&mut dot_git);
    ...
    }
    {
        ...
        {
            let caps =
                fs_capabilities.unwrap_or_else(|| gix_worktree::fs::Capabilities::probe(&dot_git));
            let mut core = config
            ...
            core.push(
                key("precomposeunicode"),
                Some(bool(caps.precompose_unicode).into()),
            );
        }
...
fn bar____EXTRACT_THIS<'lt0, 'lt1>(dot_git: &'lt0 mut PathBuf) -> NewDir<'lt1> {
    dot_git.push("info");
